...
$has_children  = false;
if ( gav( $card_options, 'show_subpages' ) ) {
...
?>
<?php if ( $has_children || ! empty( gav( $card_options, 'manual_page_ids' ) ) ): ?>
	...
	?>">
			...
			<?php if ( ! empty( gav( $card_options, 'manual_page_ids' ) ) ): ?>
				<?php
				$manual_cards = new \WP_Query( array(
					'post_type' => 'page',
					'post__in'  => (array) gav( $card_options, 'manual_page_ids' ),
					'orderby'   => 'post__in',
					// 'order' => 'ASC',
					'ignore_sticky_posts' => true,
				) );
